...
- 変更履歴
  ...
  - v0.8 主体別売買動向(§6.7)と10年国債利回り(§6.8)を実物で確認して実装。金額は千円単位、`Section` は8種類で「東証全体」は `TokyoNagoya`、公表は木曜が83%で対象週から6日後が最多。財務省の全期間 CSV は `data/` の下。ドル円は日本銀行に直接取得できる CSV が無いことが分かり、取得元を未定として記載
  - v0.9 実機に入れて見つかった2件を修正。`launchctl bootout` のあとは消えるのを待ってから `bootstrap` する(§10.3)。財務省の当月の CSV は毎回取る(§6.8。全期間版は前月までしか入っていない)
- もとにした文書
...
**2026-09-27 に実物で確認し、実装した。** 以下はすべて実測にもとづく。

- 取り方: `/equities/investor-types` を、前回のキャッシュの最後の週から今日まで取る。初回(`--rebuild`)は10年前から取る。
  - **契約の範囲外を指定すると 400 が返る。** 本文に `Your subscription covers the following dates: 2016-09-27 ~` が入るので、その日付を読み取って1回だけやり直す。
  - 履歴の長さ: **2016-09-20 の週から**(スタンダードプランは10年前まで)。
  - 流量: 100回/分に抑え、429 のときは60秒以上待つ(要件 §7)。
  - API キーは既存の `J-Quants/.env` の `JQUANTS_API_KEY` を読む。新しく置かない。キーはログに書かない。
- 17:00ルール: 公表日 `PubDate` の 17:00 より後にしか使えない。集計は、実行した時刻にすでに使える週だけを出す。
  - **`PubDate` は木曜が 83%**(1,855/2,368件)、金曜 404件、残りはまれ。
  - **公表の遅れは、対象週の最終営業日から6日後が最多**(1,772件)。7日 431件、最大18日。つまり画面では**常に1週以上前の週が最新**になる。
- **金額の単位は千円。** 出力では円に直す(`× 1000`)。同じ週・同じ区分について、投資部門別の `TotTot`(売り+買い)と、自前の売買代金×2 を6組で突き合わせ、比が 873〜1002 になった(グロースは 988・1002)。プライムがやや低いのは、投資部門別が委託+自己で東京・名古屋の合計であるため。
- 主体の対応(実物の項目名)。各主体に `Sell`・`Buy`・`Tot`・`Bal` があり、使うのは差引(`Bal`)。

| `subject` | 画面の名前 | 項目(差引) |
|---|---|---|
| `foreigners` | 海外投資家 | `FrgnBal` |
| `individuals` | 個人 | `IndBal` |
| `investment_trusts` | 投資信託 | `InvTrBal` |
| `business_cos` | 事業法人 | `BusCoBal` |
| `trust_banks` | 信託銀行 | `TrstBnkBal` |
| `proprietary` | 証券会社の自己売買 | `PropBal` |
| `other` | その他(画面では出さない) | `InsCoBal` + `BankBal` + `OthFinBal` + `OthCoBal` |

- 区分の対応(`Section`)。8種類すべてを確認した。

| `Section` | `scope` | 件数 | 期間 |
|---|---|---|---|
| `TokyoNagoya` | `all` | 520 | 2016-09-20〜(全期間) |
| `TSEPrime` / `TSEStandard` / `TSEGrowth` | `prime` / `standard` / `growth` | 各232 | 2022-04-04〜 |
| `TSE1st` / `TSE2nd` / `TSEMothers` / `TSEJASDAQ` | `legacy_*` | 各288 | 〜2022-04-01 |

  - **「東証全体」は `TokyoNagoya`(東京・名古屋の公式の合計)をそのまま使う。** 3区分を足して作らない。全期間にわたって連続しているので、市場再編をまたいでつながる。
- 項目名・区分の値の対応表は、コードに直接書かず、`aggregate/investors.py` の先頭の表にまとめる(API の形が変わったとき、そこだけ直す)。
- 出力: 週(`week_start`・`week_end`)・`section`・`subject` ごとに `balance_yen` と `pub_date`。
- **画面の要約は「値のある直近 R 週」で作る。** 公表が1週以上遅れるため、期間の右端の数週はいつも空になる。生の最後の R 週で平均を取ると、要約が常に空になった(2026-09-27 に踏んだ。§5.3)。
...
| `usdjpy` | **未定(2026-09-27 時点)**。下の「ドル円の取得元」を参照 | 同上 |
| `wti` | EIA API v2 `petroleum/pri/spt`、系列 `RWTC`(日次)。**無料の API キーが必要**(`/Users/yone/StockPortal/.env` の `EIA_API_KEY`) | 同上 |
...
#### 10年国債利回り(2026-09-27 に実装)
- URL は2つ。**全期間は `data/` の下にある**(`https://www.mof.go.jp/jgbs/reference/interest_rate/data/jgbcm_all.csv`)。当月は `.../interest_rate/jgbcm.csv`。`interest_rate/jgbcm_all.csv` は 404 になる(実際に踏んだ)。
- キーも登録も不要。Shift_JIS、`基準日,1年,2年,…,10年,…` の形。列は名前で探す。
- 和暦の日付(`R8.9.25`)を西暦にする。元年の対応: R → 2018+年、H → 1988+年、S → 1925+年。平成31年4月30日と令和元年5月1日の境目も正しく変換できることを確かめた。
- 実測: 全期間 9,929日分(1986-07-05〜2026-08-31)、当月15日分。合わせて 9,944日分。
- **当月の CSV は毎回取る(2026-09-27 修正)。** 全期間の CSV は**前月まで**しか入っていない。初回に全期間だけ取って当月を飛ばすと、今月の週が埋まらない(実際に踏んだ)。全期間の CSV は、無いときと `--rebuild` のときだけ取る。
- 取ったファイルは `portal_data/raw/mof/` にそのまま保存し、取れなかったときはそこから読み直す。

#### ドル円の取得元(**未定**。2026-09-27 に調査)
当初「日本銀行 時系列統計データ検索サイトの API」と書いていたが、**日本銀行は直接取得できる CSV を公開していない**。
- 日本銀行「外国為替市況(日次)」: 直近70営業日分は **PDF のみ**。長期は時系列統計データ検索サイトへの案内だけ。
- 時系列統計データ検索サイト: `famecgi2` のフォーム操作が前提。系列コードは `FM08'FXERM**` の形だと分かったが、安定した URL での取得方法は未確認。
- 財務省の「外国為替相場」: **外国為替平衡操作(介入実績)**のページで、為替レートそのものではない。日次のドル円は公表していない。
- みずほ銀行のヒストリカルデータ(`quote.csv`): 403 で取得できない。

決めるまでは `usdjpy` の行を出さず、`manifest.json` の `sources.boj` に理由を残す。画面はそのカードだけ空にする。
...
5. plist を `~/Library/LaunchAgents/` に置き、`launchctl bootstrap` で読み込む。すでにあれば入れ替える。
   - **`bootout` のあとは、消えるのを待ってから `bootstrap` する(2026-09-27 修正)。** 続けて実行すると `Bootstrap failed: 5: Input/output error` になる(実際に踏んだ)。`launchctl print` で消えたことを確かめ(最大10秒)、それでも失敗したら数回やり直す。登録できなければそこで止める。
...
**要確認(実物を見て決める)**
1. ~~投資部門別情報の API の項目名、`Section` の値、履歴の長さ、`PubDate` の曜日と時刻~~ → **2026-09-27 確認済み・実装済み**(§6.7)。金額は千円単位、`Section` は8種類、履歴は 2016-09-20 から、`PubDate` は木曜が83%、公表の遅れは6日が最多。
...
4. ドル円の取得元(§6.8)。**日本銀行に直接取得できる CSV は無いと分かった**(2026-09-27 調査)。取得元を決め直す必要がある。
5. 財務省 CSV の利用条件と更新の時刻(§6.8。**CSV 自体は 2026-09-27 に取得できた**)、EIA の公表の遅れ(API キーの取得が先)。
