...
- 版: 草案 v0.1(2026-09-26)
- もとにした文書
  - 基本設計書 [docs/basic-design.md](basic-design.md) v0.5
...
## 2. 技術スタック(**仮置き**。要件 §9-1 が未決のため、この文書で案を置く)
...
| Python の環境 | ポータル専用の仮想環境(`uv` で作る) | 既存の anaconda(Python 3.9)に部品を足すと、既存の分析に影響するおそれがある。既存のスクリプトは anaconda の Python で呼ぶ(§4.4) |
...
host = "0.0.0.0"
port = 8765
allowed_hosts = ["localhost", "127.0.0.1", "yone-mac.local"]   # Host ヘッダーで受け付ける名前(§9.1)
...
| `GET /api/market/japan?period=52w` | G-11 | 売買代金のカード、TOPIX と売買代金の週足、配分、主体別(東証全体)、EPS・PER、指標 |
| `GET /api/market/segments?period=52w` | G-12 | 区分ごとのシェアと PER、主体別(区分ごと) |
| `GET /api/market/sectors?period=52w&scope=all` | G-13 | 業種の一覧とヒートマップ |
...
- `period` は `13w` / `26w` / `52w`。`scope` は `all` / `prime` / `standard` / `growth`。`date` は `YYYY-MM-DD` か `latest`。
...
- `summary` は、6つとも表示しているときの要約。主体を外しても要約は変えない(**仮置き**。基本設計書は要約の対象を決めていない)。
...
- `results.jsonl`(約16MB)は、全部をメモリに持たない。最初に1回読み、銘柄コード → **最後の行**のファイル内の位置(バイト)の索引だけを作る(要件 §5.3)。根拠の API では、その位置から1行だけ読む。
- 市場概況の集計の結果(§6.9)は、全部を読んで pandas の表で持つ(全履歴でも数MB)。
- キャッシュは、ファイルごとに最新の1つだけを持つ。基準日を切り替えて過去の週を見たときの `verdicts.csv` などは、直近の8週分まで持つ(LRU。**仮置き**)。
...
- 普段(土曜の実行)は、**差分だけ**計算する。前回の結果を読み、最後の4週を計算し直して、それより新しい週を足す(遅れて公表された指標や、後から直ったキャッシュを拾うため。4週は**仮置き**)。
...
- **集計の対象から外す銘柄**(**仮置き**。要件 §9-8 が未決)
  - `Mkt` が `0109`(ETF・REIT など)、`0105`(TOKYO PRO MARKET)の銘柄
  - 銘柄一覧に載っていない銘柄(ログに件数を書く)
  - 外す区分の一覧は設定(`[aggregate] exclude_markets`)に書き、未決が決まったら設定だけで変えられるようにする。
...
3. その1件から、使う予想の値を次で決める。
   ...
   - **注意**: 翌期の予想の項目名は、要件定義書の `NxFNP` ではなく、キャッシュの実物では **`NxFNp`**(小文字の p)である(2026-09-26 に確認)。
...
| `indicator` | 取り方 | 週の値 |
|---|---|---|
| `jgb10y` | 財務省の CSV(Shift_JIS。日付は和暦 `R8.9.25` の形)。`jgbcm_all.csv` は初回だけ、以後は `jgbcm.csv`(当月)を取る。「10年」の列 | その週の最終営業日の値。なければ、その週の中でいちばん新しい日の値(**仮置き**) |
...
- 画面の「◯月◯日の週までの値」は、`obs_date` がある最後の週から作る(基本設計書 §4.10。**仮置き**)。
...
- **判定のラベルは `verdicts.csv` を正とする。** `results.jsonl` の `verdict` からは作り直さない(D2)。
  - 2026-09-25 の実物では、グランビルの「判定不能」2件が `results.jsonl` では `neutral` になっている。ラベルの決め方は一括分析の側にあるので、ポータルは CSV のとおりに出す。
- 局面の列は、そのまま文字列で持つ。
- 知らない手法の列が増えたときは、読み飛ばしてログに書く(表の列は増やさない。**仮置き**)。
...
- 根拠の API では、`results` の中の `MethodResult` を、手法ごとに次の形で返す。
  ...
  - 「判定と局面」の前提の文(例: 「週足・13週線・判定に使う範囲は直近8本」)は、`state` の値から作らず、手法ごとに決まった文を画面に持つ(**仮置き**)。`state` から作る方法は**要確認**。
...
- **指値の段・参考・検証の限界は、Markdown の表から読む。** どの段を選ぶか、落ちた理由は何かは J-Quants 側のロジックなので、`plans_*.csv` から作り直さない(D2)。
  - そのため、スプリント1では `plans_<基準日>.csv` は**読まない**(基本設計書 §7.2 との違い。§12.3)。
...
- URL に載せないもの(画面の中だけで持つ): 主体別の選んだ主体、判定表の絞り込み・ページ、根拠を開いた行、株数の目安の資金。どれも画面を離れたら消える(基本設計書 §3.1・§5.5 (5))。
...
- 1つのカードのデータの読み込みに失敗しても、ほかのカードは出す。そのため、画面の API は1本でも、画面の側ではカードごとに `null` を見て、欠けたカードだけ失敗の表示にする(**仮置き**)。
...
1. **絞り込み**(すべて AND)
   - 買い判定の数: 行ごとに、5つの手法の判定が `buy` の数を数える(`conflict` は数えない)。選んだ数以上の行を残す。**この数は表に出さない**(R1・R2)。
...
2. **並べ替え**(固定。基本設計書 §5.4 (3))
   ...
   - `rank`: `buy`=0、`neutral`=1、`unknown`=2、`conflict`=2、`sell`=3(**仮置き**)。
...
4. **銘柄の検索から来たとき**(`/weekly/verdicts?code=8306`): 絞り込みを既定にし、その行があるページを開き、その行の根拠を開いて、その行までスクロールする。対象外の銘柄なら、表の上に「この銘柄は一括分析の対象外です…」と「TradingView で開く」を出す(基本設計書 §4.5)。
   - `code` のパラメータは、この用途のためにこの文書で足す(**仮置き**)。

- 根拠を開いた行は同時に1行だけ。別の行を開いたら、前の行を閉じる。

### 8.7 TradingView へのリンク
- `https://jp.tradingview.com/chart/?symbol=TSE%3A<コード>`(**仮置き**)。
...
- 資金は保存しない(`localStorage` にも置かない。基本設計書 §5.5 (5))。

### 8.9 スマホの表示
- 切り替えの横幅は 768px(CSS の `@media (max-width: 767px)`。**仮置き**)。
- 押せるものは高さ 44px 以上。
...
| `com.yone.stockportal.weekly` | 土曜 3:00(`StartCalendarInterval`) | `python -m stockportal.batch.weekly` | `logs/weekly/<基準日>_<手順>.log` |
...
- 古いログは、26週より前のものを週次の後続の処理の最後に消す(**仮置き**)。
...
### 12.3 基本設計書・要件定義書と違うところ(確認してほしいこと)
1. **`plans_<基準日>.csv` を読まない。** 指値の段・参考の上位10銘柄・検証の限界は、`picks_<基準日>.md` の表から読む(§7.2)。段の選び方や落ちた理由は J-Quants 側のロジックで、ポータルに複製しないため(D2)。基本設計書 §7.2 の読み元の表から `plans_*.csv` を外すかどうか。
2. **決算短信の翌期の予想の項目名は `NxFNp`**(要件定義書 F1-8 は `NxFNP`)。要件定義書の表記を直すかどうか。
3. **判定のラベルは `verdicts.csv` を正とする。** `results.jsonl` の `verdict` とは一部食い違う(判定不能が neutral になっている)。根拠を開いたときの判定のラベルも `verdicts.csv` に合わせる。

### 12.4 未決・要確認(実装の前か、実装の最初に確かめる)

**未決(基本設計書 §10.4 から、作り方に関わるもの)**
1. 技術スタック(要件 §9-1)。この文書の §2 の案でよいか。
2. 売買代金の集計範囲(ETF・REIT などを含めるか)。設定で切り替えられるようにしておく(§6.4)。
